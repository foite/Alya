...
namespace utils
{
    std::string generate_klv(const std::string& protocol, const std::string& version, const std::string& rid)
    {
        ...
        static std::array constant_values = {
            sha256(md5(sha256(protocol))),
            sha256(sha256(version)),
            sha256(sha256(protocol) + salts[3]) };

        return sha256(constant_values[0] + salts[0] + constant_values[1] + salts[1] + sha256(md5(sha256(rid))) + salts[2] + constant_values[2]);
    }
